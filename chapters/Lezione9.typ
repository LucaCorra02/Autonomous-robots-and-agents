...
#figure(
  image("/assets/odometry_evaluation.png", width: 80%),
...
=== 1.1 Evaluation: Computing Pose Probabilities
...
Then, for every displacement, we compute a score by feeding the error into a Probability Density Function (PDF) $p_sigma(x) = 1 / sqrt(2 pi sigma^2) e^(-x^2 / (2 sigma^2))$, which is a zero-mean Gaussian with variance $sigma^2$.

#figure(
  image("/assets/gaussian_density_distribution.png", width: 100%),
...
- $p_1 = p_(sigma)(Delta_1)$ with $sigma^2 = alpha_1 hat(delta)_1^2 + alpha_2 hat(d)^2$
- $p_2 = p_(sigma)(Delta_2)$ with $sigma^2 = alpha_3 hat(d)^2 + alpha_4 hat(delta)_1^2 + alpha_4 hat(delta)_2^2$
- $p_3 = p_(sigma)(Delta_3)$ with $sigma^2 = alpha_1 hat(delta)_2^2 + alpha_2 hat(d)^2$

#figure(
  image("/assets/displacement_error_2.png", width: 70%),
  caption: [Computation of the displacements and error variances depending on the movement length.]
)
...
#figure(
  image("/assets/displacement_error_1.png", width: 60%),
...
=== 1.2 Sampling: Generating Random Poses

While evaluation computes a probability for a given pose, *sampling* generates a random $s_t$ distributed according to $p(s_t | s_(t-1), u_t)$. This is heavily used in the *Particle Filter* (Monte Carlo Localization).
...
#figure(
  image("/assets/odometry_sampling.png", width: 70%),
...
#note[
  ...
  $u_t = (v, omega)$
  It is like steering a car with a fixed steering wheel: the robot translates and rotates at the exact same time.
]

=== 2.1 Evaluation: The Circular Arc Assumption
...
#figure(
  image("/assets/velocity_evaluation.png", width: 50%),
...
#warning[
  The model explanation is not fully consistent with reality! A perfect circular arc dictates a fixed final heading. Without an "error buffer", the model would assign a probability of 0 to almost all actual transitions because real-world noise always causes heading drift.
]

To fix this, we introduce $hat(gamma)$ (heading drift). It represents the extra rotation needed to explain why the robot's nose isn't pointing exactly tangent to the circle.
...
#figure(
  image("/assets/probability_density_cloud.png", width: 70%),
  caption: [Probability density clouds for the velocity model showing the inclusion of heading drift to reflect real-world noise.]
)

=== 2.2 Sampling: Applying Noisy Controls
